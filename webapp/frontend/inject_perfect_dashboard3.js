...
function inject(file) {
  ...
  if (endIdx === -1) endIdx = content.indexOf('<!-- 3. Patient Doctors Page -->');
  if (endIdx === -1) endIdx = content.indexOf('<div id="patient-page-doctors"');
  ...
  if (startIdx !== -1 && endIdx !== -1) {
    ...
    console.log('Successfully injected perfect dashboard into', file);
  } else {
    console.error('Markers not found in', file, startIdx, endIdx);
  }
}

inject('c:/HealthSync/webapp/frontend/index.html');
inject('c:/HealthSync/webapp/frontend/remote_index.html');

// Fix the styling for the pixel perfect match
