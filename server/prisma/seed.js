...
async function main() {
    ...
    const passwordHash = await bcryptjs_1.default.hash('HealthSync@123', 10);
    // 1. Admin User
    ...
    for (const d of doctorsData) {
        ...
        for (const day of [client_1.DayOfWeek.MONDAY, client_1.DayOfWeek.TUESDAY, client_1.DayOfWeek.WEDNESDAY, client_1.DayOfWeek.THURSDAY, client_1.DayOfWeek.FRIDAY, client_1.DayOfWeek.SATURDAY]) {
    ...
    const patientsData = [
        {
            ...
            bloodGroup: client_1.BloodGroup.O_POSITIVE,
    ...
    for (const a of ambulanceData) {
        ...
        if (!user) {
            user = await prisma.user.create({
                data: {
                    phone: a.phone,
                    role: client_1.UserRole.AMBULANCE_OPERATOR,
                    password: passwordHash,
                },
            });
        }
        const op = await prisma.ambulanceOperator.upsert({
            ...
            create: {
                ...
                currentStatus: client_1.AmbulanceStatus.AVAILABLE,
                latitude: a.lat,
                longitude: a.lng,
            },
        });
    ...
    for (let idx = 0; idx < slotTimes.length; idx++) {
        ...
        const slot = await prisma.slot.upsert({
            ...
            create: {
                ...
                startTime: st,
                endTime: et,
                status: idx === 0 ? client_1.SlotStatus.BOOKED : client_1.SlotStatus.AVAILABLE,
            },
        });
        ...
        if (idx === 0) {
            ...
            if (!existingApt) {
                            ...
                            create: [
                                {
                                    ...
                                    dosage: '40mg',
                                    form: client_1.MedicineForm.TABLET,
                                    frequency: 'Once daily (morning)',
                                    timing: client_1.MedicineTiming.BEFORE_FOOD,
                                    duration: '30 days',
                                ...
                                },
                                {
                                    ...
                                    timing: client_1.MedicineTiming.AFTER_FOOD,
                                    duration: '30 days',
                                },
                            ],
                        },
                ...
                await prisma.medicalRecord.create({
                    data: {
                        patientId: patient.id,
                        type: client_1.MedicalRecordType.PRESCRIPTION,
    ...
    if (!existingEmg) {
        const emg = await prisma.emergency.create({
            data: {
                ...
                status: client_1.EmergencyStatus.AMBULANCE_ASSIGNED,
                initialLatitude: 18.5204,
                initialLongitude: 73.8567,
                locationTrails: {
                    create: [
                        {
                            source: client_1.LocationSource.PATIENT,
                            latitude: 18.5204,
                            longitude: 73.8567,
                            accuracy: 10,
                        },
                ...
                statusHistories: {
                    create: [
                        {
                            status: client_1.EmergencyStatus.INITIATED,
                            updatedBy: 'SYSTEM',
                            notes: 'Patient triggered Emergency SOS from mobile app',
                        },
                        {
                            status: client_1.EmergencyStatus.ACKNOWLEDGED,
                            updatedBy: 'SYSTEM',
